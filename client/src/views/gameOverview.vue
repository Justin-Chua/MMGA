...
<script>
    ...
    const updateSessionData = async () => {
        ...
        const response = await fetch("https://mmga-production.up.railway.app/game/existsTopGame?userid="+loggedInId.value+"&gameid="+route.params.id, {method: "GET"});
    ...
    const fetchLikedDiscussions = async() => {
        const response = await fetch('https://mmga-production.up.railway.app/discussion/likedDiscussionsByUserID?userid=' + loggedInId.value);
        const data = await response.json();
    ...
    export default {
        ...
        mounted() {
            ...
            let bRoute = "https://mmga-production.up.railway.app/game/getGameById?id="+ this.$route.params.id; 
            fetch(bRoute, {method: "GET"})
            .then(res => res.json())
            .then(data => this.game = data[0])
            .catch(err => console.log(err.message));

            fetch("https://mmga-production.up.railway.app/discussion/getDiscussionsByGame?id="+ this.$route.params.id, {method: "GET"})
            .then(res => res.json())
        ...
        methods: {
            ...
            addToTopGames: async function(gameid){
                const response = await fetch("https://mmga-production.up.railway.app/game/addTopGame?userid="+loggedInId.value+"&gameid="+gameid, {method: "POST"});
                if (response.status == 200){
                    alert("Added game");
            ...
            toggleLike: async function(event, discussion_id) {
                if (loggedIn.value != true) {
                    ...
                    if (button.classList.contains("unliked-button")) {
                        ...
                        const response = await fetch("https://mmga-production.up.railway.app/discussion/removeLikedDiscussion?userid=" + loggedInId.value 
                        ...
                        const response = await fetch("https://mmga-production.up.railway.app/discussion/addLikedDiscussion?userid=" + loggedInId.value 
                        +"&discussionid=" + discussion_id, { method: "POST" });
                        if (response.status != 200) {
